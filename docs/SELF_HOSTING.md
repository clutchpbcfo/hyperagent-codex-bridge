...
- In-memory idempotency replay prevents duplicates within one running process. Durable budget state prevents a restart from silently freeing committed or pending capacity, but cross-restart response replay requires platform-backed idempotency or a deliberately sensitive local response store.
